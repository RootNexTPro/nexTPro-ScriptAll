...
# 1. Importer execSync (seulement si pas déjà là)
...
# 4. Déclarer default vars au début du scope post /sync
sed -i '/const { username, password, protocol, expiry, uuid } = req.body;/a \  const finalQuota = 0;\n  const finalMaxLogins = 1;' "$FILE"

# 5. Déclarer les requêtes INSERT du post /sync
...
sed -i 's/uuidv4(), username, password || '"''"', protocol, expiry, superAdmin.id/uuidv4(), username, password || '"''"', protocol, expiry, finalQuota, finalMaxLogins, superAdmin.id/g' "$FILE"


# 6. Dans le POST /, extraire les nouvelles variables
...
# 7. Définir des valeurs par défaut pour les quotas et les connexions
...
# 8. Injecter dans createSshAccount
...
# 9. Modifier les requêtes INSERT du POST /
